	...
	*************************************************************************************/
	if(isset($_POST['button1'])){
		...
		if(empty($qtde) && empty($valor_unitario)){//verifica se o usuario preencheu todos os campos
		 ...
		 $cadquery = "INSERT INTO saida_produto (sai_id_produto,sai_qtde, sai_valor_unitario, sai_id_cliente ) VALUES ('$codigo', '$qtde','$valor_unitario', '$cliente')";
	...
	$sql_code = "SELECT  * FROM produto p INNER JOIN saida_produto sp ON p.pro_id=sp.sai_id_produto";
...
<body class="bg-dark">
  <div class="container">
    <div class="card card-register mx-auto mt-5">
      <div class="card-header">Registrar Saida de produto</div>
      <div class="card-body">
              ...
              <div class="col-md-6">
                ...
                <select name="codigo_produto">
                         <option value=""><-----SELECIONE O PRODUTO-------></option>
                          <?php 
						  $query = mysql_query("SELECT * FROM estoque e INNER JOIN produto p ON e.esto_id_produto=p.pro_id where e.esto_id");
      						while($reg=mysql_fetch_array($query)){
         					 echo '<option value="'.$reg["pro_id"].'">'.$reg["pro_id_produto"]." - ".$reg["pro_descricao"] .'</option>';}	 
						  ?>
